...
export interface UsersBoolExp {
  ...
  displayName?: Maybe<NameComparisonExp>;
  uuid?: Maybe<UuidComparisonExp>;
}
...
export interface UsersInsertInput {
  displayName?: Maybe<Scalars['name']>;
  uuid?: Maybe<Scalars['uuid']>;
}
...
export type TasksSubscriptionSubscription = (
  ...
  & { tasks: Array<(
    { __typename?: 'tasks' }
    & Pick<Tasks, 'authorId' | 'description' | 'title'>
  )> }
);
...
export const TasksSubscriptionDocument = gql`
  ...
  export class TasksSubscriptionGQL extends Apollo.Subscription<TasksSubscriptionSubscription, TasksSubscriptionSubscriptionVariables> {
    document = TasksSubscriptionDocument;
